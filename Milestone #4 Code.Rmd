...
Our project continues with the two CalEnviroScreen (CES) datasets from 2021 ("scores" and "measures") and the California Health and Human Services (CHHS) asthma emergency department (ED) visit rates dataset from 2020.
...
We have updated our data dictionary code to be more comprehensive as well as to better reflect the joined dataset.

```{r data_dictionary_updated - Kathy, echo = TRUE}
...
We have prepared the final dataset, titled data_final_with_race, by starting with the county CES measures and scores from data_final and then adding race-based asthma Emergency Department (ED) visit rate data by using the pivot wider function from the asthma dataset. Thus, data_final_with_race contains 58 observations, one for each of the 58 California counties, for the variables of interest selected from the "asthma" (CHHS), "measures" (CES), and "scores" (CES) datasets.

In addition, we created a dataframe titled race_data_state that shows the statewide asthma ED visit rates by race. The asthma ED visit rates among Black residents is the highest at 89.9 per 10,000 residents, which is almost 5 times as high as the asthma ED visit rate for White residents at 18.3 per 10,000 residents. This data demonstrates that Black Californians are at higher risk of being affected by severe asthma exacerbations requiring hospitalizations compared to other races. Therefore, public health interventions must be done to alleviate this disproprotionate burden. Our team will continue to explore the impacts of race on asthma and its implications for developing interventions.

```{r final_join_and_pivot - Vig & Jaemie, echo = TRUE}
...
In this next section, we have prepared visualizations based on our selected variables of interest and the asthma ED visit rates and/or the CES measures. We hope these tables and charts will help us communicate our data analysis findings to others interested in the relationship between asthma, environmental exposures, and socioeconomic measures.
...
#### Chart #1: CES Score & Asthma

Here is a scatterplot that aims to identify the relationship between CES 4.0 Score and Adult Asthma ED Visit Rates. A regression line has been added at the 95% confidence level to help identify any patterns between pollution and asthma visits. There are two counties that have data only from the CES datasets but none from the CHHS asthma dataset, #1 Alpine County, and #2 Sierra County.

There appears to be a slight positive relationship between CES score and adult asthma visit rates: counties who are measured to have higher environmental and socioeconomic burdens are associated with increased adult asthma ED visits.

```{r chart_1_scatterplot_ces_vs_asthma - Jaemie, echo = TRUE}
...
#### Chart #2: Pollution & Asthma

This scatterplot aims to identify the relationship between Pollution Burden Score and Adult Asthma ED Visit Rates. Again, a regression line is included at the 95% confidence level, and the two counties without asthma ED visit rate data are excluded (Alpine County and Sierra County).

There does not seem to be a clear relationship between pollution burden and asthma ED visit rates. Therefore, pollution burden may be interacting with asthma ED visit rates in a non-straightforward way. Other variables should be explored to determine what factors play a larger role in asthma visit rates.

```{r chart_2_scatterplot_pollution_vs_asthma - Jaemie, echo = TRUE}

filtered_data_final_with_race <- subset(data_final_with_race, !county %in% c("ALPINE", "SIERRA"))

scatterplot_pollution_vs_asthma <- ggplot(filtered_data_final_with_race, 
       aes(
         x = as.numeric(`Pollution Burden Score`), 
         y = as.numeric(`Adult Age-Adjusted ED Visit Rate`))) +
  geom_point() +
  ggtitle("2020 California Adult Asthma Emergency Department Visit Rate and 
          CES 4.0 Pollution Burden Score, by County", "Data from California Health and Human Services (CHHS) and CalEnviroScreen (CES)") +
  xlab("Pollution Burden Score") +
  ylab("Adult Asthma ED Visit Rates per 10,000 residents") +
  labs(caption = "Regression line is included with the gray area at the 95% confidence level") +
  geom_smooth(method="auto", se=TRUE, fullrange=FALSE, level=0.95)

scatterplot_pollution_vs_asthma
```

#### Chart #3: Housing Burden & Asthma

This next scatterplot aims to identify the relationship between Housing Burden Percent and Adult Asthma ED Visit Rates. Again, a regression line has been added at the 95% confidence level, and counties with no visit rate data (Alpine County and Sierra County) are excluded.

Yet again, there does not seem to be a clear relationship between housing burden and asthma ED visit rates, suggesting that other variables may be more of a factor.

```{r chart_3_scatterplot_housing_vs_asthma - Jaemie, echo = TRUE}

scatterplot_housing_vs_asthma <- ggplot(filtered_data_final_with_race, 
       aes(
         x = as.numeric(`Housing Burden %`), 
         y = as.numeric(`Adult Age-Adjusted ED Visit Rate`))) +
  geom_point() +
  ggtitle("2020 California Adult Asthma Emergency Department Visit Rate and 
          CES 4.0 Housing Burden %, by County", "Data from California Health and Human Services (CHHS) and CalEnviroScreen (CES)") +
  xlab("Housing Burden Percent") +
  ylab("Adult Asthma ED Visit Rates per 10,000 residents") +
  labs(caption = "Regression line is included with the gray area at the 95% confidence level. Housing Burden Percentage refers to 
  the percentage of households that are both low income (making less than 80% of their county's median family 
  income) and severely burdened by rent (paying greater than 50% of their income toward housing costs).") +
  geom_smooth(method="auto", se=TRUE, fullrange=FALSE, level=0.95)

scatterplot_housing_vs_asthma 
```

#### Chart #4: Age & Asthma

Below is a boxplot created to compare total number of ED visits due to asthma between children (0-17 years old) and adults (\>18years old) across all California counties in 2019. The general trend appears to show overall more ED visits in adults (\>18 years of age) than that of children (0-17 years of age).

```{r chart_4_box_plot_adults_vs_children_asthma - Kathy, echo = TRUE}
...
boxplot <- ggplot(asthma_2020_adult_vs_child, aes(x=age_group, y=number_of_ed_visits, fill = age_group)) +
  ...
  ggtitle("Comparison of 2020 Asthma-Related Emergency Department Visits 
          between Children and Adults Across All California Counties", "Data from California Health and Human Services (CHHS)") +
...
The bargraph below compares the number of ED visits due to asthma between different race groups across all California counties. Based on the comparison chart, the race that appears to be disproportionately affected by asthma to a point of requiring emergency department visits are Hispanic followed by Black.
...
We hope these visualizations will help us communicate our findings of the relationships between asthma and environmental or socioeconomic factors. Our team will review these findings and likely revise our analysis to better capture any potential relationships. After further adjustments and research, we hope that our findings can be used to inform asthma-related public health interventions in California and reduce health disparities among age, race, and socioeconomic factors. To make a meaningful impact on the health of Californians with asthma, interventions should be evidence-based and tailored to help the populations experiencing the highest burdens.
